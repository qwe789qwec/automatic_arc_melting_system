...
sliderxrd::sliderxrd(std::string ip, int port)
{
    // Connect to the slider controller
    sliderxrd_tcp.connect(ip, port);
    
    // Activate all required servos
    sliderxrd_tcp.write(servo_onf(MOTOR_1, ON));
    sliderxrd_tcp.check_receive("#992322C", 3);
...
std::string sliderxrd::command(std::string command)
{
	// Calculate checksum
	uint8_t sum = 0;
...
std::string sliderxrd::servo_onf(std::string station, std::string state)
{    
    // Create servo on/off command: !992320101@@\r\n
    return command("!99232" + station + state);
}

std::string sliderxrd::servo_move(std::string station, std::string position, std::string speed)
{    
...
std::string sliderxrd::status(std::string station)
{
    // Create status request command: !9921201@@\r\n
    return command("!99212" + station);
}

std::string sliderxrd::length2string(int number)
{
...
std::string sliderxrd::count_circle(double radius, double number, double index, std::string center)
{
...
void sliderxrd::check_position(std::string servo)
{
    ...
    while (true) {
        sliderxrd_tcp.write(status(servo));
        sliderxrd_tcp.receive(message);
...
void sliderxrd::move(std::string servo, std::string position, std::string speed)
{
    // Send move command
    sliderxrd_tcp.write(servo_move(servo, position, speed));
    sliderxrd_tcp.check_receive("#99234", 3);
...
void sliderxrd::curve_move(std::string servo1, std::string servo2, std::string position, std::string speed)
{
...
void sliderxrd::arc_path(int direction, int times, int stop_times)
{   int count_dir = 0;
...
{    
    ...
    else if(action == "xrdpos"){
        move(MOTOR_1, length2string(XRD_POS), "100");
    }
    else {
        return false;
    }
...
sliderxrd::~sliderxrd()
{
    ...
    sliderxrd_tcp.write(servo_onf(MOTOR_1, OFF));
    sliderxrd_tcp.check_receive("#992322C", 3);
    sliderxrd_tcp.write(servo_onf(MOTOR_2, OFF));
    sliderxrd_tcp.check_receive("#992322C", 3);
    sliderxrd_tcp.write(servo_onf(MOTOR_3, OFF));
    sliderxrd_tcp.check_receive("#992322C", 3);
    sliderxrd_tcp.write(servo_onf(MOTOR_X, OFF));
    sliderxrd_tcp.check_receive("#992322C", 3);
    sliderxrd_tcp.write(servo_onf(MOTOR_Y, OFF));
    sliderxrd_tcp.check_receive("#992322C", 3);
    sliderxrd_tcp.write(servo_onf(MOTOR_Z, OFF));
    sliderxrd_tcp.check_receive("#992322C", 3);
