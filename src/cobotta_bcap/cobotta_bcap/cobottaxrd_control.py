...
class CobottaXRDClient(Node):

    def __init__(self):
        super().__init__('cobottaxrd_client')
...
class CobottaXRDSubscriber(Node):

    def __init__(self):
        super().__init__('cobottaxrd_subscriber')
        ...
        self.last_process = "cobottaxrd_init"
    ...
    def listener_callback(self, msg):
        ...
        if msg.process != self.last_process:
            ...
            cobotta_handle = cobottaxrd(host,port,timeout)
            action = cobotta_handle.make_action(msg.process)
            cobotta_client = CobottaXRDClient()
            if action != "error":
                response = cobotta_client.send_request("cobottaxrd_standby")
...
def main(args=None):
    ...
    cobotta_subscriber = CobottaXRDSubscriber()
